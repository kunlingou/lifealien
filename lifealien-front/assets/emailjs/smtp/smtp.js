...
import { monitor } from './response';
import SMTPError, { CONNECTIONAUTH, COULDNOTCONNECT, BADRESPONSE, NOCONNECTION, AUTHFAILED, AUTHNOTSUPPORTED } from './error';
...
class SMTP extends EventEmitter {
	...
	connect(callback, port = this.port, host = this.host, options = {}) {
		...
		const connectedErrBack = err => {
			if (!err) {
				...
				caller(
					callback,
					SMTPError('could not connect', COULDNOTCONNECT, err)
				);
			}
		};
	...
	send(str, callback) {
		if (this.sock && this._state === SMTPState.CONNECTED) {
			...
			caller(
				callback,
				SMTPError('no connection has been established', NOCONNECTION)
			);
		}
	}
		...
		const response = (err, msg) => {
			if (err) {
				...
				if (codesArray.indexOf(Number(msg.code)) !== -1) {
					...
					caller(
						callback,
						SMTPError(errorMessage, BADRESPONSE, null, msg.data)
					);
				}
			}
		};
	...
	login(callback, user, password, options) {
		...
		const initiate = (err, data) => {
			...
			const failed = (err, data) => {
				...
				caller(
					callback,
					SMTPError('authorization.failed', AUTHFAILED, err, data)
				);
			};
			...
			switch (method) {
				...
				default:
					...
					const err = SMTPError(msg, AUTHNOTSUPPORTED, null, data);
					caller(callback, err);
					break;
			}
		};
...
const _SMTP = SMTP;
export { _SMTP as SMTP };
export const state = SMTPState;
export const authentication = AUTH_METHODS;
export const DEFAULT_TIMEOUT = TIMEOUT;
